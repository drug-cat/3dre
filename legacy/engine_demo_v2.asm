...
section .data
align 16
className db "EngineDemoV2",0
windowTitle db "Engine Demo v2 - C/V add, Delete remove, 1/2 mesh, M wireframe",0
...
.find_slot:
    ...
    jmp .done
.found:
    mov dword [rdi], 0          ; type = cube
    mov dword [rdi+4], 0x3F800000 ; 
    mov dword [rdi+4], 0         ; x=0
    mov dword [rdi+8], 0         ; y=0
    mov dword [rdi+12], 0        ; z=0
.done:
    add rsp, 0x20
    pop rdi
    pop rsi
...
.find_slot2:
    ...
    jmp .done2
.found2:
    mov dword [rdi], 1          ; type = sphere
    mov dword [rdi+4], 0
    mov dword [rdi+8], 1        ; y=1
    mov dword [rdi+12], 0
.done2:
    add rsp, 0x20
    pop rdi
...
draw_sphere:
    ...
    mov esi, [sphereStacks]  
    mov edi, [sphereSlices]   
...
.object_empty:
    ...
    ; Draw shadows for each object (just cube for simplicity)
    lea rsi, [objectList]
    mov ecx, MAX_OBJECTS
.draw_shadows_loop:
    mov eax, [rsi]
